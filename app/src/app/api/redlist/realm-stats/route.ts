...
/**
 * Per-realm totals across ALL species — the Realm view's landing cards read this
...
export async function GET() {
  try {
    const realms = await getRealmStats();
    return NextResponse.json({ realms }, { headers: CACHE_1H });
  } catch (error) {
